    ...
    return (
        <div className="about-us-into">
            {aboutusInto?.headings[0] && (
                <h3 className="font-bold mb-5 text-primary pl-24 relative after:absolute content-after after:bg-primary after:w-16  after:h-1 after:z-0  after:top-1/2  after:left-0  after:transform  after:-translate-y-2/4  after:transition  after:opacity-100">
                    {aboutusInto?.headings[0]?.content}
                </h3>
            )}
            {aboutusInto?.headings[1] && (
                <h2 className="text-blue font-bold  mb-6">
            ...
            <div className="about_desc mb-8">
                {aboutusInto?.items &&
                    aboutusInto.items.map((item) => (
                        <p className="text-blue leading-8" key={item.id}>
                            {item.description}
                        </p>
                    ))}
            </div>
